...
export default function Param() {
  ...
  return (
    ...
    >
            <div className={`text-center p-4 max-w-md transition-opacity duration-75 ease-out ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
        <h1 className="text-2xl font-bold mb-2">{list[currentIndex]?.title}</h1>
        <p className="text-md mb-2">{list[currentIndex]?.description}</p>
        <div className="code-section text-sm p-2 rounded mb-4 text-left whitespace-pre-wrap break-all overflow-x-hidden overflow-y-auto max-h-90 bg-gray-800 rounded-md" onTouchStart={(e) => e.stopPropagation()} onTouchMove={(e) => e.stopPropagation()} onTouchEnd={(e) => e.stopPropagation()}>
